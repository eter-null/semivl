...
class SemiDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'val':
            # Ensure mask is single-channel class index map for metrics
            mask_np = np.array(mask)
            if mask_np.ndim == 3:
                # Heuristic for binary masks: any non-zero becomes class 1
                mask_np = (mask_np.sum(axis=-1) > 0).astype(np.uint8)

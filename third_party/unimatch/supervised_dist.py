    ...
    if mode == 'padded_sliding_window':
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                # Normalize model output to [B, C, H, W]
                if crop_seg_logit.dim() == 4 and crop_seg_logit.shape[1] != num_classes and crop_seg_logit.shape[-1] == num_classes:
                    # Likely [B, H, W, C]
                    crop_seg_logit = crop_seg_logit.permute(0, 3, 1, 2).contiguous()
        ...
        # Use the aggregated prediction at the original image resolution
        final = preds
        pred = final.argmax(dim=1)
        ...
        final = model(img)
        # Normalize model output to [B, C, H, W]
        if final.dim() == 4 and final.shape[1] != cfg['nclass'] and final.shape[-1] == cfg['nclass']:
            final = final.permute(0, 3, 1, 2).contiguous()
        pred = final.argmax(dim=1)
